...
function main({obj, size, source}) {
    ...
    if (startsWith(msg, "time=") && tags.Some( (_, tag) => startsWith(tag, "date=" ))){
       // regular fortigate
    } elseif (startsWith(msg, "timestamp=") && tags.Some( (_, tag) => startsWith(tag, "logver=" ))){
       // fortianalyzer
    } else {
       return {"status":"abort"}
    }
